...
ggplot(test, aes(y = PGS, x = RateRatio)) +
  ...
  ylab(" ") + theme_classic() +
  ggtitle('Figure 12:PGS and ntPGS with number of hospitalizations (SZ)') +
  theme(plot.title = element_text(hjust = 0.4))

ggsave(filename = 'figure12_prs_ntprs_num_hosp_sz.tiff', units = 'in', width = 7.5, height = 5, dpi = 700, device = 'tiff')

table10 <- as_grouped_data(x = test, groups = c('type')) %>% flextable::as_flextable(col_keys = c('Genotype', 'RRCI', 'pvalue'))%>% 
  ...
  add_header_lines('Table  10: Proband PGS, ntPGS and number of hospitalizations in Schizophrenia cohort') %>% 
...
table10 %>%save_as_image(path = 'table10.png')
